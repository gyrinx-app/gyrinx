...
def check(page, url, width, screenshot_dir=None, theme="light"):
    ...
    if width < 1024:
        ...
        browse = page.get_by_role("button", name="Browse help", exact=True)
        expect(browse).to_be_visible()
        browse.focus()
        browse.press("Enter")
        drawer = page.get_by_role("dialog", name="Browse help", exact=True)
...
def main():
    ...
    with sync_playwright() as playwright:
        ...
        page = context.new_page()
        page.goto(args.url)
        for label in ("Browse help", "On this page"):
