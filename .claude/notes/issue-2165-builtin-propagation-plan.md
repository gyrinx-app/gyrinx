...
refusals D11/D12 were part of C6's scope and are also unbuilt; decide
separately whether they still want doing.

*Superseded plan below, kept for the shape if it is ever revived:*
...
*Status: MERGED 2026-08-31 (PR #2351, squash aa29f0dfe). Review round found one real bug before merge: `_tag_legacy_grants` could bind a legacy copy to an ARCHIVED set member when a live twin named the same thing, leaving the live member unsatisfied so catch-up granted a silent duplicate — members now claim in `("archived", "pk")` order (archived stay taggable: a sold grant's tag is what keeps it from being handed back), regression-tested. Also: cause-less legacy rows are counted into `unmatched` so the report explains every row the preview shows. Declined with reasons: twin pairing order (provenance names the pair, every later read goes through it), counting `omit` per assignable (over-omitting under-grants and self-reports), recording an operator on CAUGHT_UP grants (library propagation carries an empty actor by design; the operator is on the Backfill row). Known wrinkle: a crash between a gang's summary write and the cursor advance replays that gang — work is idempotent, only census counts can overstate. RUN IN PROD 2026-08-31 (record 0fe12692): 1,846 gangs walked, 0 failures, several deliveries, cursor handed back cleanly each time. Totals: 24,359 tagged, 10,694 already accounted for by a tagged copy, 959 granted, 1 ambiguous, 68 unmatched, 5 held another way, 0 skipped. Outside verification (read-only, independent of the run's own counts): tagged + already + unmatched + ambiguous = 35,122 = the preview's taggable count exactly; 959 CAUGHT_UP events in the run window = the granted figure; zero grants with non-zero paid or rating_contribution; zero grants missing provenance or cause; zero duplicate (member, carrier) pairs anywhere in the estate; all 455 live carriers of the nested subtype now hold both of its counters (0 missing, was 894 missing). The 5 held-another-way cases are 2 of one profile already holding a rule another way and 3 grenade lines already under their launcher array — the ammo-under-gun rule working. Audit re-run 2026-08-31 (record b7612606): 1,849/1,849 gangs, 0 failures, no drift recorded — the estate's books fold clean after the backfill. C7 COMPLETE. Next chunk: C8 (tighten: remove `_granted_rows_without_provenance`). C6 dropped in favour of the removal warning (PR #2364).*
