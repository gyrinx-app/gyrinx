        ...
        if campaign.status == "pre_campaign":
            ...
            for resource in CampaignListResource.objects.filter(campaign=campaign):
                ...
                CampaignAction.objects.create(
                    ...
                    user=system_user,
                    list=resource.list,
                    description=f"Resource '{resource.resource_type.name}' was automatically removed{old_amount_msg} for gang '{resource.list.name}' because the campaign is in pre-campaign status. This resolves a Gyrinx-internal issue.",
                    owner=system_user,
                )
