...
@pytest.mark.django_db
def test_stat_override_preview_is_dry_run(make_user, make_list, make_list_fighter):
    """GET renders the plan and writes nothing."""
    from n23.core.models.list import ListFighterStatOverride

    superuser = make_user("c2super", "pw")
    superuser.is_staff = superuser.is_superuser = True
    superuser.save()
    _fighter_statline_type()

    lst = make_list("Preview Gang")
    fighter = make_list_fighter(lst, "Preview Fighter")
    fighter.toughness_override = "5"
    fighter.save()

    client = Client()
    client.force_login(superuser)
    response = client.get(reverse("admin:maintenance_migrate_stat_overrides"))

    assert response.status_code == 200
    fighter.refresh_from_db()
    assert fighter.toughness_override == "5"
    assert not ListFighterStatOverride.objects.exists()
    assert not Backfill.objects.filter(
...
@pytest.mark.django_db
def test_stat_override_migration_refuses_while_a_template_lacks_a_statline(
    make_user, make_list, make_list_fighter, content_fighter
):
    """Those fighters read the legacy column directly — the column is the card."""
    superuser = make_user("c2super2", "pw")
    superuser.is_staff = superuser.is_superuser = True
    superuser.save()

    lst = make_list("Blocked Gang")
    fighter = make_list_fighter(lst, "Blocked Fighter")
    fighter.toughness_override = "5"
    fighter.save()
...
@pytest.mark.django_db
def test_stat_override_migration_records_a_backfill(
    make_user, make_list, make_list_fighter, content_fighter
):
    from n23.content.models.statline import ContentStatline, ContentStatlineStat
    from n23.core.models.list import ListFighterStatOverride

    superuser = make_user("c2super3", "pw")
    superuser.is_staff = superuser.is_superuser = True
    superuser.save()
    statline_type = _fighter_statline_type()
    statline = ContentStatline.objects.create(
        content_fighter=content_fighter, statline_type=statline_type
    )
    for type_stat in statline_type.stats.all():
        ContentStatlineStat.objects.create(
            statline=statline, statline_type_stat=type_stat, value="-"
        )

    lst = make_list("Migrating Gang")
    fighter = make_list_fighter(lst, "Migrating Fighter")
    fighter.toughness_override = "5"
    fighter.save()

    client = Client()
    client.force_login(superuser)
    response = client.post(reverse("admin:maintenance_migrate_stat_overrides"))

    assert response.status_code == 302
    record = Backfill.objects.get(operation=Backfill.Operation.MIGRATE_STAT_OVERRIDES)
    assert record.status == Backfill.Status.DONE
    assert record.triggered_by == superuser
    assert record.summary["applied"] == 1
    fighter.refresh_from_db()
    assert fighter.toughness_override is None
    assert ListFighterStatOverride.objects.get(list_fighter=fighter).value == "5"
