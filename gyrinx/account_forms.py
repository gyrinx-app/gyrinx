"""Platform account forms.

The allauth overrides named in ``settings.ACCOUNT_FORMS`` plus the
username-change form. None of these touch edition models.

``BadgeSelectionForm`` is NOT here yet: it reads ``UserProfile``, which is still
an edition model. It joins this module when that model moves to the platform.
"""
