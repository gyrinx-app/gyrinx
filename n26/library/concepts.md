...
**Asset type** — *a class of asset a campaign type has: Territory, Settlement.* A row on the campaign type, edited on its page: a label (singular and plural, the plural defaulting to an s), an **ownership**, and a position. Its assets are listed under it on the same page, and added there. The ownership is on the asset type, not the asset, because a whole class behaves one way. **Possession**: every gang has its own (a Settlement, a home territory). **Holding**: one gang holds it at a time, and it can change hands (a Territory). Two asset types of one campaign type cannot share a label, and an asset type cannot be removed while any asset is of it.
