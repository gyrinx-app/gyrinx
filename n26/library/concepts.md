...
Fields of its own: a **description** and its **asset types** (see below). The description is written for the arbitrator setting a campaign up: what the gangs fight over, what each starts with, how the campaign runs and how it ends. The set-up screen draws it on the type's card, beside sentences the app composes from the type's asset types, built-ins and modifiers. It is the one field on a campaign type a player reads; the author help stays for authors. Under each asset type sit the **assets** of that type — the list of what a campaign of this type hands out. An asset belongs to one asset type, and so to one campaign type; there is no separate list on the type.

Assignable for the same reason a gang type is: joining a campaign is a gang-hosted assignment naming the type. That gives everything a member gang receives on joining — a Reputation counter with its opening value, a Settlement — something to be caused by, and gives campaign-wide modifiers a carrier every member's card can find. On the type's page that list is headed "Every gang that joins gets". A counter or a rule is added to it by hand; an asset of an inherent asset type is added to it when the asset is created, and taken out when the asset is deleted, so the author never adds one. Its pricing fields stay at zero; nobody buys a campaign type.

Shared types live in the system pack. The one that ships is the **Territory campaign**, the core rulebook's own: gangs fight for control of Territory, every gang keeps a Settlement and starts with one Territory, and each Territory held gives a Boon. The campaign has three phases (Occupation, Downtime, Takeover) and ends with Triumphs. Its asset types are Settlement (inherent) and Territory (transferable); it has a Settlement asset, and Reputation at 0, Income at 0 and the Settlement built in. What an arbitrator adds to one campaign — a counter, an asset type, a label — goes on a second campaign type in that campaign's own pack, layered on the shared one rather than copied from it. That second type is the campaign's machinery: the authoring lists leave it out, and the campaign page shows what it holds as tables and columns beside the shared type's.

**Asset type** — *a class of asset a campaign type has: Territory, Settlement.* A row on the campaign type, edited on its page: a label (singular and plural, the plural defaulting to an s), an **ownership**, and a position. Its assets are listed under it on the same page, and added there. The ownership is on the asset type, not the asset, because a whole class behaves one way. **Inherent**: every gang has its own (a Settlement, a home territory). **Transferable**: one gang holds it at a time, and it can change hands (a Territory). Two asset types of one campaign type cannot share a label, and an asset type cannot be removed while any asset is of it.

### Asset

*One thing a campaign has — a Settlement, the Old Ruins territory — of one asset type.*

One entry in the list of what a campaign type hands out. It is made on the campaign type's page, under one of the type's asset types, and that is the only way it belongs to the campaign type. Assets have no listing or create page of their own; an asset's own page, reached from the campaign type's, is where its modifiers and the rest of its fields are edited.

Fields of its own: its **asset type** (settled when the asset is made, and fixing both the campaign type it belongs to and how it behaves). What holding the asset does for its holder — Reputation while held, a special rule, a free hire — rides it as ordinary modifiers. Its **income** is one of them: the Income box on the authoring forms writes a modifier that adds the figure to the holder's **Income** counter, a system counter every gang in a Territory campaign has at 0. A gang's Income is therefore the sum of what it holds, and is drawn wherever its counters are. Nothing collects the reading; nothing moves credits.

Assignable so that an inherent asset can be built into its campaign type and arrive on every member gang, and so that an asset of either ownership can carry modifiers. An inherent asset is built in as it is created, taken out when it is deleted or archived, and built in again when an archived one is brought back; gangs already given one keep it. It is built into its asset type's campaign type, or, for an asset an arbitrator makes in a campaign's own pack, into that campaign's own type. A transferable asset is never assigned: the campaign's own record of the asset says who holds it, and the gang's history says when it gained or lost it.

### Asset table

*A table of one transferable asset type's assets — the Territory Selection Table — that a gang holds.*

Made on the campaign type's page, under one of its transferable asset types; an inherent asset type has no tables, because every gang has its own of those and there is nothing to roll for. A table's own page, reached from the campaign type's, lists its **entries** — one asset each, of the table's asset type — and, on a rolled table, the band of rolls that lands on each. Above the entries the page says whether the bands cover the die: every roll the die can make should land on exactly one entry, and a gap or an overlap is named there.
...
Fields of its own: a **plural** (what several of them are called, so a page can say "Gang Legacies"), **allows repeats** (whether one holder may pick the same pickable for two slots of this type), and **is lasting effect** (whether carried slots of this type are offered when recording post-battle results).
