...
Assignable for the same reason a gang type is: joining a campaign is a gang-hosted assignment naming the type. That gives what every member gang is given on joining — a Reputation counter with its opening value, a Settlement — something to be caused by, and gives campaign-wide modifiers a carrier every member's card can find. On the type's page that list is headed "Every gang that joins gets". A counter or a rule is added to it by hand; an asset of a Possession asset type joins it the moment the asset is created, and leaves it when the asset is deleted, so the author never adds one. Its pricing fields stay at zero; nobody buys a campaign type.

Shared types live in the system pack. The one that ships is the **Territory campaign**, the core rulebook's own: gangs fight for control of Territory, every gang keeps a Settlement and starts with one Territory, and each Territory held gives a Boon. The campaign has three phases (Occupation, Downtime, Takeover) and ends with Triumphs. Its asset types are Settlement (a possession) and Territory (a holding); it has a Settlement asset, and Reputation at 0, Income at 0 and the Settlement built in. What an arbitrator adds to one campaign — a counter, an asset type, a label — goes on a second campaign type in that campaign's own pack, layered on the shared one rather than copied from it. That second type is the campaign's machinery: the authoring lists leave it out, and the campaign page shows what it holds as tables and columns beside the shared type's.

**Asset type** — *a class of asset a campaign type has: Territory, Racket, Settlement.* A row on the campaign type, edited on its page: a label (singular and plural, the plural defaulting to an s), an **ownership**, and a position. Its assets are listed under it on the same page, and added there. The ownership is on the asset type, not the asset, because a whole class behaves one way. **Possession**: every gang has its own (a Settlement, a home territory). **Holding**: one gang holds it at a time, and it can change hands (a Territory, a Racket, a Relic). Two asset types of one campaign type cannot share a label, and an asset type cannot be removed while any asset is of it.

### Asset

*One thing a campaign has — a Settlement, the Old Ruins territory, a Racket — of one asset type.*
...
Assignable so that a possession can be built into its campaign type and arrive on every member gang, and so that an asset of either ownership can carry modifiers. A possession is built in as it is created — into its asset type's campaign type, or, for an asset an arbitrator makes in a campaign's own pack, into that campaign's own type — and taken out when it is deleted or archived; gangs already given one keep it. A holding is never assigned: the campaign's own record of the asset says who holds it, and the gang's history says when it gained or lost it.
