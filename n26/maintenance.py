...
class Operation(models.TextChoices):
    ...
    OPEN_FOUNDING_ACTIONS = (
        ...
        "n26: the Spend built-in TP action is opened on gangs that never had one",
    )
    DELETE_LEGACY_AFFILIATION_ASSIGNMENTS = (
...
def open_founding_actions(backfill_id, **said_by_whoever_enqueued_it):
    """Open a Spend built-in TP action on every unarchived gang that
    ...
    if request.method == "POST":
        ...
        if not eligible:
            messages.info(
                ...
                "has the Spend built-in TP action.",
            )
            return HttpResponseRedirect(address)
...
register_operation(
    MaintenanceOperation(
        ...
        added=date(2026, 9, 4),
        description=(
            "Open the Spend built-in TP action on every unarchived "
