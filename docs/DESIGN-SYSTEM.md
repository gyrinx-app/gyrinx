...
An admin can tick **Show contents** on a page's options (the "Flat page options" inline on the flat page admin) to render a nested list of its headings above the content: `{% page_contents flatpage %}` in `flatpages/default.html`. The list carries no bullets or numbers and nests by indent alone, 1em per level, matching the sidebar page nav. Heading ids are slugs of their text, de-duplicated with `-2`, `-3`… so the contents links always resolve.
