...
@pytest.mark.django_db
def test_form_without_orientation_saves_portrait(client, user, make_list):
    lst = make_list("Gang")
    client.force_login(user)
    ...
    assert resp.status_code == 302
    config = PrintConfig.objects.get(list=lst, name="Plain")
    assert config.orientation == PrintConfig.PORTRAIT


@pytest.mark.django_db
def test_orientation_param_prints_landscape_without_config(
    client, user, make_list, make_list_fighter
):
    """A crew print has no config, so the address can ask for landscape."""
    lst = make_list("Gang")
    make_list_fighter(lst, "Grimjaw")
    client.force_login(user)

    body = client.get(_print_url(lst) + "?orientation=landscape").content
    soup = BeautifulSoup(body, "html.parser")
    assert soup.select_one("#content")["data-orientation"] == "landscape"

    body = client.get(_print_url(lst) + "?style=classic&orientation=landscape")
    assert "print-sheet--landscape" in body.content.decode()


@pytest.mark.django_db
def test_orientation_param_overrides_the_config(
    client, user, make_list, make_list_fighter
):
    lst = make_list("Gang")
    make_list_fighter(lst, "Grimjaw")
    ...
    body = client.get(_print_url(lst, config) + "&orientation=portrait").content
    soup = BeautifulSoup(body, "html.parser")
    assert soup.select_one("#content")["data-orientation"] == "portrait"


@pytest.mark.django_db
def test_unknown_orientation_param_falls_back_to_the_config(
    client, user, make_list, make_list_fighter
):
    lst = make_list("Gang")
    make_list_fighter(lst, "Grimjaw")
    ...
    body = client.get(_print_url(lst, config) + "&orientation=sideways").content
    soup = BeautifulSoup(body, "html.parser")
    assert soup.select_one("#content")["data-orientation"] == "landscape"


@pytest.mark.django_db
def test_classic_lore_notes_print_takes_the_orientation_param(client, user, make_list):
    lst = make_list("Gang")
    client.force_login(user)
    url = reverse("core:list-print-lore-notes", kwargs={"id": lst.id})

    body = client.get(url + "?style=classic&orientation=landscape").content.decode()
    assert "print-sheet--landscape" in body
    assert "size: A4 landscape" in body

    body = client.get(url + "?style=classic").content.decode()
    assert "print-sheet--landscape" not in body


@pytest.mark.django_db
def test_grouped_cards_carry_group_print_columns(client, user, make_list):
    """A blank vehicle card is a crew and its vehicle side by side."""
    lst = make_list("Gang")
    config = _config(lst, user, blank_vehicle_cards=1)
    client.force_login(user)

    soup = BeautifulSoup(client.get(_print_url(lst, config)).content, "html.parser")
    group = soup.select_one("#content .print-col-group")
    assert group is not None
    assert len(group.select(".card.print-col-in-group")) == 2
